...
export default function Savings() {
  ...
  function load() {
    api.listSavings(token).then(setGoals).catch(() => {})
  }
  ...
  async function createGoal(e) {
    ...
    try {
      await api.createSavings(token, { name, targetAmount: Number(target) || 0 })
      setName(''); setTarget('')
      load()
    } catch (err) {
  ...
  async function contribute(id) {
    ...
    await api.contributeSavings(token, id, { amount })
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col gap-2 md:flex-row md:items-baseline md:justify-between">
        ...
        <p className="font-mono text-xl text-brass">{formatMoney(totalSaved)} <span className="text-xs text-ink/40 font-body">saved in total</span></p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {goals.length === 0 && <p className="text-ink/40 text-sm">No savings goals yet — create one above.</p>}
        {goals.map((g) => {
